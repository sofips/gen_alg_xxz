...
def plot_normalized_couplings(files, labels=False, title=False, size=(8, 5), save=False):
 '''
...
def plot_all_samples(directory, dimension, fid_limit=0., title=False, ns=10, transmission_time=False, delta=1.0, beta_is_gene=False):
...
def plot_energy_differences(files,  couplings=True, directory = '', delta = 1.0, labels=False):
    '''
    ...
    for i in range(len(files)):
        ...
        for i in range(1, n):
            ...
            diff[i] = abs(DeltaE[i]*(n)/np.pi -
                          closest_odd_integer(DeltaE[i]*n/np.pi))
            if diff[i] < 0.05:
                count += 1
...
def contour(df, column='fitness', title=False):
    ...
    Z = df.pivot_table(index='dimension', columns='delta',
                       values=column).T.values
    ...
    cnt = plt.contourf(X, Y, Z,  levels=20, cmap=cm.turbo,
                       vmin=vmin, vmax=vmax)
...
def compare_mean(dataframes, title=False, attribute='fitness', labels=False, figsize=[12, 5], save=False):
    ...
    for i in range(len(dataframes)):
        ...
        plt.errorbar(stats['dimension'], stats['mean'], yerr=stats['std'],
                     fmt='o-', label=label, capsize=4, zorder=-2)
    plt.grid(True)
    plt.legend()
    plt.xlabel('N')
...
def summary_graph(df, title=False, figsize=[15, 5], ylim_min=0.9, ylim_max=1., save=False):
    ...
    stats = pd.DataFrame({'mean fitness': mean_fitness, 'sd fitness': std_fitness, 'min fitness': min_value_fitness,
                          'max fitness': max_value_fitness, 'mean generations': mean_gen, 'sd generations': std_gen, 'min generations': min_value_gen,
                          'max generations': max_value_gen, 'mean time': mean_time, 'sd time': std_time, 'min time': min_value_time,
                          'max time': max_value_time})

    stats = stats.reset_index()

    plt.errorbar(stats['dimension'], stats['mean fitness'], yerr=stats['sd fitness'],
                 fmt='o', label='average fidelity', capsize=4, zorder=-2)

    maxdf = df.groupby('dimension')['fitness'].max()
    filtered_max = df[df['fitness'].isin(maxdf)]
    filtered_max = filtered_max.reset_index()

    plt.plot(filtered_max['dimension'], filtered_max['fitness'], '--',
             label='max fidelity', linewidth=1.5, color='black', zorder=-1)

    sc1 = plt.scatter(filtered_max['dimension'], filtered_max['fitness'],
                      c=filtered_max['generations'], cmap='rainbow')

    for j, txt in enumerate(filtered_max['generations']):
        plt.annotate(
            txt, (filtered_max['dimension'][j], filtered_max['fitness'][j]))

    min_j = df.groupby('dimension')['fitness'].min()
    filtered_min = df[df['fitness'].isin(min_j)]
    filtered_min = filtered_min.reset_index()

    plt.plot(filtered_min['dimension'], filtered_min['fitness'], '--',
             label='min fidelity', linewidth=1.5, color='grey', zorder=-1)

    sc2 = plt.scatter(filtered_min['dimension'], filtered_min['fitness'],
                      c=filtered_min['generations'], cmap='rainbow')

    for i, txt in enumerate(filtered_min['generations']):
        plt.annotate(
            txt, (filtered_min['dimension'][i], filtered_min['fitness'][i]))
    ...
    vmin = min(filtered_max['generations'].min(),
               filtered_min['generations'].min())
    vmax = max(filtered_max['generations'].max(),
               filtered_min['generations'].max())
    ...
    axs[0].errorbar(stats['dimension'], stats['mean time'],
                    yerr=stats['sd time'], fmt='o', label='average', capsize=4)
    axs[0].plot(stats['dimension'], stats['max time'],
                '--o', label='max', linewidth=2.5)
    axs[0].plot(stats['dimension'], stats['min time'],
                '--o', label='min', linewidth=2.5)
    ...
    axs[1].errorbar(stats['dimension'], stats['mean generations'],
                    yerr=stats['sd generations'], fmt='o', label='average', capsize=4)
    axs[1].plot(stats['dimension'], stats['max generations'],
                '--o', label='max', linewidth=2.5)
    axs[1].plot(stats['dimension'], stats['min generations'],
                '--o', label='min', linewidth=2.5)
...
def access_best_solutions(dataframe, directory, dimensions, return_labels = True, extra_label=' '):
    '''
    ...
    for dimension in dimensions:
        ...
        max_fitness_solution_j_1 = directory + '/jn' + \
            str(dimension)+'sample'+str(max_value_index_j_1)+'.dat'

        label = extra_label + 'fidelity =' + \
            str(max_value_fitness_j_1) + ', N = ' + str(dimension)
...
def population_histogram(ga):
    ...
    isExist = os.path.exists('hist_frames')
    if not isExist:
        os.mkdir('hist_frames')
    ...
    # save to file
    filename = 'hist_frames/hist_frame'+str(ng).zfill(3)+'.png'
